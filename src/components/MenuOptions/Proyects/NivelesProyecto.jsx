...
export default function NivelesProyecto({ id, SubirLevel }) {
  ...
  const HandleSUbumit2 = async (e) => {
    e.preventDefault();
    //setLoading(true);
    alert("Ver data");
    console.log("users, idpro" + id);
    console.log(users);
    /*
    setLoading(true);
    e.preventDefault();
  ...
  return (
    <div className="relative overflow-y-auto h-screen mx-auto">
      {/**<button onClick={HandleSUbumit2}>Ver data</button> */}
      ...
      <div className="relative h-full mb-16 mx-auto">
        ...
        {users.map(
          (
            { r_id_area, r_nombre_area, r_nivel, r_tipo_nivel, r_estado_nivel },
            index
          ) => (
            <div
              className="mx-auto p-4 w-80 "
              style={{ top: r_nivel * 150 }}
              key={index}
            >
              <div className="items-center flex flex-col gap-1 mb-10 bg-blue-gray-50 cursor-pointer hover:border-4 hover:border-green-800">
                ...
                {index !== 0 && (
                  <Chip
                    ...
                    color={r_estado_nivel === "Sin enviar" ? "red" : "green"}
                    value={r_estado_nivel}
                  />
                )}
              </div>
              {r_nivel >= 0 && r_nivel <= users.length - 2 ? (
                <ArrowDownIcon

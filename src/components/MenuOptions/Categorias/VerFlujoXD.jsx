...
export default function VerFlujoXD({ cerrar, id }) {
  ...
  const load2 = async () => {
    ...
    const result = await fetch(
      process.env.NEXT_PUBLIC_ACCESLINK + "proyects/VerFlujoCategoria/" + id,
      {
        method: "GET",
          ...
          <div className="h-72 relative">
            <div className="relative h-screen mx-auto">
              <div className="relative h-full mb-16 mx-auto">
                {users2.length === 0 ? "No hay flujo para cargar" : ""}
                {users2.map((task, index) => (
                  <div
                  ...
                  >
                    ...
                    {task.numero >= 0 && task.numero <= users2.length - 2 ? (
                      <ArrowDownIcon
